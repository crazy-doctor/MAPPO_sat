...
class PPO_agent(object):
	def __init__(self, args, state_dim, action_dim):
		# state_dim action_dim dvc a_lr T_horizon entropy_coef entropy_coef_decay
		# gamma
		...
		self.T_horizon = args.traj_len
		self.dvc = args.device
		self.args = args
		# Choose distribution for the actor
		if self.Distribution == 'Beta':
			self.actor = BetaActor(self.state_dim, self.action_dim).to(self.dvc)
		elif self.Distribution == 'GS_ms':
			self.actor = GaussianActor_musigma(self.state_dim, self.action_dim).to(self.dvc)
		elif self.Distribution == 'GS_m':
			self.actor = GaussianActor_mu(self.state_dim, self.action_dim).to(self.dvc)
		else: print('Dist Error')
		self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=self.args.actor_lr)

		# Build Critic
		self.critic = Critic(self.state_dim).to(self.dvc)
	...
	def train(self):
		...
		done = torch.from_numpy(self.done_hoder).to(self.dvc)  # 任务成功或者失败以及到达一局中允许的最大时间
		dw = torch.from_numpy(self.dw_hoder).to(self.dvc)  # 任务成功或者中途死亡
		...
		with torch.no_grad():
			...
			deltas = r + self.args.gamma * vs_ * (~dw) - vs
			deltas = deltas.cpu().flatten().numpy() # 优势
			adv = [0]

			'''done for GAE'''
			for dlt, mask in zip(deltas[::-1], done.cpu().flatten().numpy()[::-1]):
				advantage = dlt + self.args.gamma * self.args.lambd * adv[-1] * (~mask)
		...
		a_optim_iter_num = int(math.ceil(s.shape[0] / self.args.a_optim_batch_size))
		c_optim_iter_num = int(math.ceil(s.shape[0] / self.args.c_optim_batch_size))
		for i in range(self.args.K_epochs):
			...
			for i in range(a_optim_iter_num):
				index = slice(i * self.args.a_optim_batch_size, min((i + 1) * self.args.a_optim_batch_size, s.shape[0]))
				...
				surr2 = torch.clamp(ratio, 1 - self.args.eps_clip, 1 + self.args.eps_clip) * adv[index]
				a_loss = -torch.min(surr1, surr2) - self.args.entropy_coef * dist_entropy
			...
			for i in range(c_optim_iter_num):
				index = slice(i * self.args.c_optim_batch_size, min((i + 1) * self.args.c_optim_batch_size, s.shape[0]))
	...
	def save(self, path, timestep):
		if not os.path.exists(path):
			os.makedirs(path)
		torch.save(self.actor.state_dict(), path+"/actor{}.pth".format(timestep))
		torch.save(self.critic.state_dict(), path+"/critic{}.pth".format(timestep))
