...
from history_reward_obs.reward_2025_7_15 import reward_obs_done as rod
...
class scenario(Tool):
    ...
    def step(self, action,noise=False):
        ...
        red_done = self.rod.red_done(assign_res=self.assign_res, inf=self.sim.inf)
        blue_done = self.rod.blue_done(assign_res=self.assign_res, inf=self.sim.inf)

        red_dead_win = red_done
        blue_dead_win = blue_done
